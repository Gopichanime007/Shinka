...
function openSettingsModal() {
  ...
  overlay.innerHTML = `
    <div class="modal" style="max-width:440px;">
      <div class="modal-header">
        ...
        <button class="icon-btn" data-act="close" aria-label="Close settings">
      ...
      <div class="modal-body">
        <p class="text-sm text-secondary">
          Everything here lives only in this browser's local storage.
          Use these options to clear or restore your workspace data.
        </p>
  ...
  overlay
    ...
    .addEventListener('click', async () => {
      const ok = await PT_UI.confirmDialog({
        ...
        message:
          'This removes every project and task stored in this browser. Your team member list stays, so you can keep assigning new tasks. This can’t be undone.',
  ...
  overlay
    .querySelector('[data-act="reseed"]')
    .addEventListener('click', async () => {
      const ok = await PT_UI.confirmDialog({
        title: 'Restore sample data?',
        message:
          'This replaces everything currently in your workspace with the original demo projects and tasks.',

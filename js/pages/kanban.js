(async function () {
  ...
  content.innerHTML = `
    ...
    <div class="toolbar fade-in">
      ...
      <select
        class="select"
        id="filterKAssignee"
        data-tooltip="Filter by assignee"
        style="${showAssignee ? '' : 'display:none;'}"
      >
      ...
      <div class="modal modal-lg">
        ...
        <div class="modal-body">
          ...
          <div class="form-row">
            <div class="field">
              <label>Project</label>
              <select class="select" id="f_project"></select>
            </div>
            ...
            >
              ...
              <select class="select" id="f_assignedTo"></select>
            </div>
          </div>
          ...
          <div class="form-row">
            ...
            <div class="field">
              ...
              <select class="select" id="f_priority"></select>
            </div>
          </div>
        ...
        <div class="modal-footer">
          <button class="btn btn-secondary" id="taskModalCancel">
            Cancel
          </button>

          <button class="btn btn-primary" id="taskModalSave">
  ...
  const defaultStatusLabel =
    statuses.find(s => s.is_default)?.label ||
    statuses[0]?.label ||
    'Open';
  ...
  projects.forEach(p => {
    filterProject.insertAdjacentHTML(
      'beforeend',
      `
        <option
          value="${PT_UI.escapeHtml(p.id)}"
        >
          ${PT_UI.escapeHtml(p.name)}
        </option>
      `
    );
  });
  ...
  members.forEach(m => {
    filterAssignee.insertAdjacentHTML(
      'beforeend',
      `
        <option
  ...
  function filteredTasks() {
    return tasks.filter(t =>
      ...
      (
        ...
        t.assignedTo === filterAssignee.value
      )
    );
  }
      ...
      return `
        <div class="kanban-col">
          <div class="kanban-col-header">
            ...
            <button
              class="kanban-col-add"
              data-addto="${PT_UI.escapeHtml(status)}"
              data-tooltip="Add task to ${PT_UI.escapeHtml(status)}"
              aria-label="Add task to ${PT_UI.escapeHtml(status)}"
            >
          ...
          <div
            ...
            data-status="${PT_UI.escapeHtml(status)}"
          >
            ${colTasks.length
          ? colTasks.map(t => cardHtml(t)).join('')
          : '<div class="kanban-empty-col">No tasks here</div>'
        }
          </div>
        </div>
      `;
  ...
  function cardHtml(t) {
    ...
    const projectName = proj?.name || t.project || '';
    const taskName = t.name || '';
    const taskId = t.id || '';
    const dueText = dueLabel(t);

    return `
      <div
        class="kanban-card"
        draggable="true"
        data-id="${PT_UI.escapeHtml(taskId)}"
      >
        <div class="kanban-card-project">
          ${PT_UI.escapeHtml(projectName)}
        </div>

        <div
          class="kanban-card-title"
          data-tooltip="${PT_UI.escapeHtml(taskName)}"
        >
          ${PT_UI.escapeHtml(taskName)}
        </div>

        <div class="flex items-center gap-2">
          ${PT_UI.priorityFlag(t.priority)}
        </div>

        <div class="kanban-card-footer">
          <span
            class="kanban-due-pill ${dueClass(t)}"
          >
            ${PT_UI.escapeHtml(dueText)}
          </span>
  ...
  function wireDragAndDrop() {
    ...
    document.querySelectorAll('.kanban-col-body').forEach(col => {
      ...
      col.addEventListener('drop', async e => {
        ...
        await PT_STORE.updateTaskStatus(
          id,
          newStatus
        );
        ...
        PT_UI.toast(
          'success',
          'Task moved',
          `“${PT_UI.escapeHtml(task.name || '')}” is now in ${PT_UI.escapeHtml(newStatus)}.`
        );
      });
    });
  }
  ...
  document
    ...
    .addEventListener('click', e => {
      ...
      openModal(
        null,
        addBtn.dataset.addto
      );
    });

  [filterProject, filterAssignee].forEach(el => {
  ...
  members.forEach(m => {
    selAssignee.insertAdjacentHTML(
      'beforeend',
      `
        <option
          value="${PT_UI.escapeHtml(m.id)}"
  ...
  statuses.forEach(s => {
    selStatus.insertAdjacentHTML(
      'beforeend',
      `
        <option
          value="${PT_UI.escapeHtml(s.label)}"
        >
          ${PT_UI.escapeHtml(s.label)}
        </option>
      `
    );
  });
  ...
  priorities.forEach(p => {
    selPriority.insertAdjacentHTML(
      'beforeend',
      `
        <option
          value="${PT_UI.escapeHtml(p.label)}"
        >
          ${PT_UI.escapeHtml(p.label)}
        </option>
      `
    );
  });
  ...
  function openModal(task = null, defaultStatus = null) {
    ...
    document.getElementById('f_estimatedHours').value =
      task?.estimatedHours ?? '';

    document.getElementById('f_actualHours').value =
      task?.actualHours ?? 0;
    ...
    .addEventListener('click', async () => {
      ...
      const payload = {
        ...
        description: document
          .getElementById('f_description')
          .value
          .trim(),

        project: selProject.value,

        assignedTo: showAssignee
          ? selAssignee.value
          : null,
